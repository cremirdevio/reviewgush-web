import { Avatar, Box, BoxProps, Drawer, DrawerContent, DrawerOverlay, Flex, HStack, Heading, Icon, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Text, VStack, useDisclosure, useMediaQuery } from '@chakra-ui/react'
...
export default function SideNavigationContent({ width = ["300px"], maxWidth } : SideNavigationContentProps) {
    ...
    return (
        <Flex width={width} maxW={maxWidth} maxH={"100vh"} height={"100vh"} flexDir={"column"} alignItems={"flex-start"} justifyContent={"space-between"} borderRight={"1px"} borderColor={"gray.200"} bg={"white"}>
            <Box px={"16px"} py={"24px"} width={"full"} overflowY={"auto"} >
                ...
                <HStack pos={"sticky"} top={0} bg={"white"} pb={"2"}>
                    ...
                    <Menu>
                        <MenuButton as={Box}>
                            <HStack>
                                <Text fontWeight={"bold"} fontSize={"md"}>Joseph Ajibodu</Text>
                                <Icon as={FaChevronDown} boxSize={"3"} />
                            </HStack>
                            <Text fontWeight={"light"} fontSize={"xs"}>josephajibodu@gmail.com</Text>
                        </MenuButton>
                        <MenuList>
            ...
            <HStack borderTop={"1px"} borderColor={"gray.200"} py={"3"} px={"16px"} width={"full"} justifyContent={"space-between"}>
                <HStack>
                    <Avatar
                        size='sm'
                        name='Cremir'
